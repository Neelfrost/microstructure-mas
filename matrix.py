...
class Matrix2D:
    ...
    def __init__(self, cols, rows, orientations, seed_method):
        self.cols = cols
        self.rows = rows
        self.orientations = orientations

        assert seed_method in Matrix2D.seed_methods, "Invalid seed method"
        self.seed_method = seed_method if seed_method in Matrix2D.seed_methods else None

        print("Starting matrix creation.")
        self._profile_start = perf_counter()
        self.grid = self.make_grid()
        self._profile_end = perf_counter()
        print(f"Completed matrix creation in {(self._profile_end - self._profile_start):.4f}s.")
        print("Populating matrix.")
        self._profile_start = perf_counter()
        self.create_grains()
        self._profile_end = perf_counter()
        print(f"Matrix populated in {(self._profile_end - self._profile_start):.4f}s.")
            ...
            for j in range(self.rows):
                if self.grid[i][j] == 0:
                    ...
                    for seed in self.seed_loc:
                        distance_between_seed_cell = self.distance_between_cells((i, j), seed)
